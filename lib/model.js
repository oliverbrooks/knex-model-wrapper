...
/**
 * generate a hook object comprising of each type of
 * hook as the keys and an array of hooks as the actions
 * @param  {Object} hooks object describing all the hooks
 * @returns {Object}       configured hook object
 */
function generateHooks (hooks) {
  ...
  return [
    ...
    out[name] = hooks[name] ? [].concat(hooks[name]) : []
    return out;
  }, {});
}

/**
 * Validate the modelDefinition has the correct keys and throw if incorrect
 * @param  {Object} modelDefinition modelDefinition
 * @returns {Void}                 no return
 */
function validateModelDefinition (modelDefinition) {
  if (!modelDefinition) {
    throw new Error("modelDefinition required");
  }
...
/**
 * Model instance
 * @param    {ModelGenerator}   generator           ModelGenerator instance
 ...
 */
function Model (generator, modelDefinition) {
  if (!generator) {
    throw new Error("ModelGenerator instance required");
  }
  ...
  this.schema = lodash.assign({}, generator.baseSchema, modelDefinition.schema);
  this.validator = generator.hannibal.create({
    type: "object",
    schema: this.schema
  });
...
/**
 * Run any appropriate hooks and return the original data
...
Model.prototype._runAfterHooks = function executeAfterHooks (action, data) {
  if (this.afterHooks[action]) {
    return this._runHooks([].concat(this.afterHooks.all).concat(this.afterHooks[action]), lodash.clone(data), action);
  } else {
    return data;
  }
};
...
Model.prototype._runBeforeHooks = function executeBeforeHooks (action, data) {
  if (this.beforeHooks[action]) {
    return this._runHooks([].concat(this.beforeHooks.all).concat(this.beforeHooks[action]), lodash.clone(data), action);
  } else {
    return data;
  }
};
 ...
 * @param {Object} data     data to pass to hooks
 * @param {String} action   database action taking place
 * @returns {Object} data
 */
Model.prototype._runHooks = function runHooks (hooks, data, action) {
  if (hooks) {
    return Bluebird.map(hooks, function (hook) {
      return hook.call(this, data, action);
    })
    .bind(this)
    .then(function () {
      return data;
